...
В продакшене (Cloudflare Pages) тот же относительный путь `/api/*` проксируется серверной функцией `functions/api/[[path]].ts` на backend. Адрес берётся из переменной окружения `BACKEND_URL`, а если она не задана — используется значение по умолчанию `http://144.31.141.142:8080`. Прокси на стороне сервера нужен потому, что Telegram Mini App открывается по HTTPS, а backend отвечает по HTTP — прямой запрос из браузера был бы заблокирован как mixed-content.
